...
- 2026-09-17 — **First direct-Claude review (VScriptEd round 2, commit fe08847): found 3 real bugs** that both the executed checks AND the orchestrator's integration review missed — (1) empty/whitespace dB input: `Number("")` and `Number("   ")` are 0, not NaN, so a cleared field silently committed unity gain (the orchestrator's own one-shot "fix" of the worker's patch still had the whitespace variant — the executed check only proved compilation, not input-edge semantics); (2) `GET /thumbs/{index}` served cached JPEGs straight off disk, bypassing the mtime invalidation the metadata endpoint runs → stale frames after retranscode/reimport; (3) the filmstrip refetch effect was keyed on `track.id` only, so a same-id reimport never re-triggered the fetch. Lesson: the review layer is load-bearing — UI-input edge cases and cache-invalidation paths are exactly what executed checks (compile + fixture tests) don't cover. Keep the per-wave direct-Claude review; brief it to scrutinize input-edge handling and cache invalidation specifically.
...
## VScriptEd timeline P4 wave 1 (2026-09-17, code-feature, vscripted-timeline-p4 + -clipfix)

All-local run per user directive. 3 tasks: layout-autosave (27B), offset-numeric-entry (9B pygmy), clip-library-integrate (9B Air).

- **vllm/qwen3.8-27b** — layout-autosave: PASS attempt 1 (~15 min). Debounced 2.5 s auto-save of the layout draft in Transcript.tsx, coexisting with the T4 resync effects; spec prescribed the exact debounce semantics and T5 one-snapshot-per-save reasoning.
- **local-llama/qwen3.8-9b (pygmy)** — offset-numeric-entry: PASS attempt 1. Second consecutive small single-file UI numeric-entry task on pygmy (db-gain round 2 needed attempt 2) — the controlled-text pattern (gainText precedent in-file) makes these mechanical for 9B.
- **local-llama-macbookair/qwen3.8-9b-gguf (Air)** — clip-library-integrate: FAIL attempt 2 (run 1): attempt 1 = TS errors (duplicated peaksForTrack), attempt 2 = TS fixed but left the rules-of-hooks lint errors (early-return before useState/useEffect) + an infinite-fetch effect (dep on a fresh array ref) + wrong m:ss format. Then run 2 (clipfix, exact-content spec): attempt-1 worker output was fully correct and its own build+lint green — but Ringer's CHECK `npm ci` died on a persistent ENOTEMPTY (rc=190) in the worktree's node_modules/lucide-react; attempt 2 hit the same infra failure. Orchestrator recovered manually: cleared the worktree node_modules (`rm -rf node_modules/*`), fresh `npm ci`, build+lint green, exported the patch, integrated.

**Routing update:** Air 9B on a 2-file React state task = 2 failures across 2 runs (model + infra). Keep 9Bs strictly single-file/mechanical; multi-file React-state tasks go to 27B or get split into single-file tasks. 27B first-try on a fully-specified frontend feature (autosave) is now a 2nd data point (with d10 round 2) — "fully-specified gnarly" is 27B's lane.

**Infra lessons (new):**
1. `npm ci` ENOTEMPTY rc=190 in ringer worktrees is persistent, not transient — a stale/corrupt node_modules subdir (lucide-react) blocks re-runs. Recovery: clear the worktree's node_modules and re-c. If the worker's own verification was green and only the check's `npm ci` fails, complete the check manually (clear → ci → build → lint → patch export) instead of burning more model attempts on infra.
2. Main-checkout backend suite MUST run as `uv run python -m pytest tests -q` — bare `uv run pytest` fails collection ("No module named 'backend'": the pytest entrypoint doesn't add CWD to sys.path) AND collects the ringer worktree copies under `data/` (hundreds of phantom collection errors). Always pass `tests` explicitly.
3. Ringer run state keeps only the final attempt's check output (confirmed again: attempt-1 failure detail lost) — keep fixing this in ringer.py.

Integration: all 3 patches applied clean; one-line review fix (offset commit `throw err` in a void-called async handler → revert-without-throw, matching the dB commit convention). Verified: frontend tsc/vite/oxlint 0 errors; backend 573 passed / 1 skipped. Not yet committed (awaiting the user's Claude review per the established flow).

Also produced `docs/TIMELINE-V2-TRIM-SPIKE.md` (VScriptEd): un-parks the §6.4 per-track in/out trim — trim_start_s/trim_end_s, timeline→file source-window math, the four ripple sites (audio mix, video compose, session duration, align/retranscribe), 5 open questions with recommended decisions, 3-task build order. Remaining P4 after this wave: loop region, ruler gap click-toggle, word-click→scroll, trim build.
